...
//track elements
        ...
        $("#submit").click(function (e) {
            ...
            tracksArr.length = 0;
            socket.emit('addToPlaylist', {artistName: artistName, trackName: trackName});
        });

        $("#interpret").keyup(function (e) {

            socket.emit('searchArtist', {artistName: e.target.value, trackName: undefined});


        });
        $("#track").keyup(function (e) {
            socket.emit('searchTrack', {artistName: undefined, trackName: e.target.value});
    ...
    socket.on('artistData', (msg) => {
        ...
        for (let item  in msg.body.artist.items) {
            let element = msg.body.artist.items[item];
